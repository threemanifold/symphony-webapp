#!/usr/bin/env bash
# Manage the Symphony "## Codex Workpad" comment on a Linear issue.
# Subcommands:
...
case "$cmd" in
    ...
    existing=$(printf '%s' "$resp" | jq -r '.data.issue.comments.nodes[]? | select(.body | startswith("## Codex Workpad")) | .id' | head -n1)
